Shader "istarnion/test/WireFrame"
{
	Properties
	{
		_LineColor ("Line Color", Color) = (1,1,1,1)
		_GridColor ("Grid Color", Color) = (1,1,1,0)
		_LineWidth ("Line Width", float) = 0.2
		_Outline   ("Outline", Range(0, 1)) = 0.07
	}
	SubShader
	{
		Pass
		{
			Tags { "RenderType" = "Transparent" }
			Blend SrcAlpha OneMinusSrcAlpha
			AlphaTest Greater 0.5
 
			CGPROGRAM
			#pragma vertex vert
			#pragma fragment frag
 
			uniform float4 _LineColor;
			uniform float4 _GridColor;
			uniform float _LineWidth;
			uniform float _Outline;

			// vertex input: position, uv1, uv2
			struct appdata
			{
				float4 vertex : POSITION;
				float4 texcoord1 : TEXCOORD0;
				float4 color : COLOR;
				float3 normal : NORMAL;
			};
 
			struct v2f
			{
				float4 pos : POSITION;
				float4 texcoord1 : TEXCOORD0;
				float4 color : COLOR;
				float3 normalDir : TEXCOORD1;
				float4 posWorld : TEXCOORD2;
			};
 
			v2f vert (appdata v)
			{
				v2f o;
				o.posWorld = mul(_Object2World, v.vertex);
				o.normalDir = normalize(mul(float4(v.normal, 0.0), _World2Object).xyz);
				o.pos = mul( UNITY_MATRIX_MVP, v.vertex);
				o.texcoord1 = v.texcoord1;
				o.color = v.color;
				return o;
			}
 
			fixed4 frag(v2f i) : COLOR
			{
				fixed4 answer;
 
				float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);

				float lx = step(_LineWidth, i.texcoord1.x);
				float ly = step(_LineWidth, i.texcoord1.y);
				float hx = step(i.texcoord1.x, 1.0 - _LineWidth);
				float hy = step(i.texcoord1.y, 1.0 - _LineWidth);
 
				if(dot(viewDirection, i.normalDir) < _Outline)
				{
					answer = lerp(_LineColor, _GridColor, lx*ly*hx*hy)+_LineColor;
				}
				else
				{
					answer = lerp(_LineColor, _GridColor, lx*ly*hx*hy);
				}

				return answer;
			}
			ENDCG
		}
	} 
	Fallback "Vertex Colored", 1
}